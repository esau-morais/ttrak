...
function inferPriorityFromLabels(labels: Array<{ name: string }>): Task["priority"] {
  const labelNames = labels.map(l => l.name.toLowerCase())
  if (labelNames.some(n => n.includes("urgent") || n.includes("critical"))) return "urgent"
...
export async function fetchGitHubIssues(config: GitHubConfig, since?: string): Promise<GitHubResponse> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      ...
      throw new Error(`Rate limit exceeded. Resets at ${resetTime ? new Date(parseInt(resetTime) * 1000).toISOString() : "unknown"}`)
    }
...
export function transformGitHubIssueToTask(issue: GitHubIssue, repo: string): Task {
...
export async function syncGitHub(config: GitHubConfig, lastSync?: string): Promise<Task[]> {
  try {
    const response = await fetchGitHubIssues(config, lastSync)
    const issues = response.data.filter(issue => !issue.pull_request)
    return issues.map(issue => transformGitHubIssueToTask(issue, config.repo))
  } catch (error) {
